...
resultsdataA = readmatrix('Z:/Andre/New FBA Results/WTC11D0VsD4FVA_ATPall_ctrl.csv');
%resultsdataB = readmatrix('Z:/Andre/New FBA Results/WTC11D0_Turneretal_NormVsmTeSRpluspalmitateConstrainFVA.csv');
%resultsdataB = readmatrix('Z:/Andre/New FBA Results/WTC11mTeSRvsmTeSRminusglut_allexchconstraintest3_11.csv');
%resultsdataB = readmatrix('Z:/Andre/New FBA Results/WTC11D0_Tohyamaetal_NormVsmTeSRnoglucglutConstrainFVA.csv');
resultsdataB = readmatrix('Z:/Andre/New FBA Results/WTC11mTeSRnoglucvsmTeSRnoglucglut_allexchconstraintest2FVA1.csv');
...
subsysrxnIDvec2 = subsysrxnIDvec(:,1:end-4);
...
%subsysLog2FCvalsrealboth1 = [(subsysLog2FCvalsrealA(1:40,:)) (subsysLog2FCvalsrealB(1:40,:))];
subsysLog2FCvalsrealboth =[subsysLog2FCvalsrealA subsysLog2FCvalsrealB];
% csandakgdh = [];
% csandakgdh(1,1) = subsysLog2FCvalsrealB(3,1);
% csandakgdh(2,1) = subsysLog2FCvalsrealB(4,1);
...
%[h,p] = ttest(PPPmaxvals1B,PPPmaxvals2B,'Alpha',0.05)
%[h,p] = ttest2(csandakgdh,subsysLog2FCvalsrealB,'Alpha',0.05)
...
%fclose(f_fva);
yvalues1 = subsysrxnIDvec2';
yvalues = subsysrxnIDvec';
xvalues = {'Log2FC Tohyama ctrl vs (-)glc','Log2FC Tohyama ctrl vs (-)glc/gln'};
...
%h = heatmap(xvalues,yvalues1,subsysLog2FCvalsrealboth1,'Colormap', hot);
h = heatmap(xvalues,yvalues,subsysLog2FCvalsrealboth,'Colormap', hot);