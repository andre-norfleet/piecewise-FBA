...
%added module_glc 07/11/22 Riya 

    % get list of available media files
    media_dir = dir('../data/media/output/*.csv'); % all .csv files in media/output directory
    available_media = {};
    ...
    %if module_glc == 1  %added glc 07/11/22 by Riya
    %    exchange_rxn = [exchange_rxn,{'EX_glc_d[e]'}];
    %end
        ...
        if length(model.rxns{i}) >= 3
            if strcmp(model.rxns{i}(1:3),'EX_') % if the first 3 characters of the rxn descriptive string ID contain exchange denotation
                if ~any(strcmp(exchange_rxn,model.rxns{i}))
                    model.lb(i) = 0;
                    %model.ub(i) = 0.00000000001;
                else
                    if any(strcmp(media_constraint_exchange,model.rxns{i})) %finds where in stoichiometric matrix EX_metab_[e] located
