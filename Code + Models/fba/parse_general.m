...
%input_file = 'Y:/Riya/From Andre/FBA-pipeline-master-Riya/Code + Models/fba/input/WTC11mTeSRmedsensATPm15/_TEMPLATE_1.xlsx'
    % load data
    ...
    if type_of_analysis == 7
        ...
        if ~isnan(value)
            if ischar(value)
                media_sensitivity_spectrum_metabolite3 = value;
            else
                error('ERROR - General - Options - Media sensitivity - Spectrum - Metabolite - Metabolite 2 name must be a string')
            end
        end
        if ~exist('media_sensitivity_spectrum_metabolite3','var')
            media_sensitivity_spectrum_metabolite3 = NaN;
        end
        
        % number of concentration points
        value = raw(23,6); %Dre 5 to 6
        value = value{1};
        if ~isnan(value)
            if isfloat(value)
                if mod(value,1) == 0
