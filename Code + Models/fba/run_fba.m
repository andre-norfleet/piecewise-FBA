...
%added module_glc 07/11/22 by Riya
    ...
    for i = 1:length(samples_all)
        ...
        %model.ub(strcmp(model.rxns,'EX_lac_L[e]')) = 40;
        ...
        %custexchrxnlistnew = model.rxns(exchrxnindex(:)); % returns vector with indicies denoting location of med. comp. exch rxn
        ...
        %letzsee = finalgroupindices{1,20}(1,3);
        %letzsee2 = finalgroupindices{1,20}(1,:); %returns a 'double' vector 1 x n that contains indices of metabs ex cyto trnpt
        %sofar = 4;

        %for j = 1:length(custexchrxnlistnew)
             %model.ub(strcmp(model.rxns, custexchrxnlistnew(j))) = 0; %lower bound, no uptake
        %    model.lb(strcmp(model.rxns, custexchrxnlistnew(j))) = -fincustomconcvaluevector(j); %07/29/22 upper bound changed to media concentration
             %model.ub(strcmp(model.rxns, custtransportrxnindices{1,j})) = fincustomconcvaluevector(j);
        %end

        %for j = 1:length(custexchrxnlistnew)
        %    model.ub(strcmp(model.rxns, custtransportrxnindices{1,j}{1,:})) = fincustomconcvaluevector(j);
             %model.ub(strcmp(model.rxns, custtransportrxnindices{1,j}(1,:))) = .02;
        %end

        %implement SeaHorse/Bioprofile FLEX2 Constraints
        PPR_reactions_noexch = {'H2td','LDH_Lm','LDH_L'};
        hydrogen_exchangerxn = {'EX_h[e]'};
        for j = 1:length(PPR_reactions_noexch)
            model.ub(strcmp(model.rxns,PPR_reactions_noexch{j})) = 1;
        end
        model.lb(strcmp(model.rxns,hydrogen_exchangerxn)) = -1;
        %oxygen uptake
        oxygenuptake_reactions_noexch = {'O2tm','O2t'};
        for j = 1:length(oxygenuptake_reactions_noexch)
            model.ub(strcmp(model.rxns,oxygenuptake_reactions_noexch{j})) = 1;
        end
        %model.lb(strcmp(model.rxns,'EX_o2[e]')) = -1;
        %ATP synthesis
         %model.ub(strcmp(model.rxns,'ATPS4')) = tbc;
        % CO2 transport
        co2transptrxns = {'CO2tm','CO2tm','CO2tg','CO2ter'};
         %co2exchrxn = {'EX_co2[e]'};
        for j = 1:length(co2transptrxns)
            model.ub(strcmp(model.rxns,co2transptrxns{j})) = 1;
        end
         %model.ub(strcmp(model.rxns,'EX_co2[e]')) = 1;
         %model.ub(strcmp(model.rxns,'EX_nh4[e]')) = 1;
         %model.lb(strcmp(model.rxns,'EX_o2[e]')) = -1;
        [wholeexchrxnlist1, exchlistallindex] = exchangerxnlist();
        wholeexchrxnlist1horiz = wholeexchrxnlist1';
        for j = 1:length(wholeexchrxnlist1horiz)
            %model.ub(strcmp(model.rxns,wholeexchrxnlist1horiz{j})) = 0.00000000001;
            %model.lb(strcmp(model.rxns,wholeexchrxnlist1horiz{j})) = -0.001;
            model.lb(strcmp(model.rxns,wholeexchrxnlist1horiz{j})) = -1;
        end
        model.ub(strcmp(model.rxns,'EX_co2[e]')) = 1;
        model.ub(strcmp(model.rxns,'EX_nh4[e]')) = 1;
        model.lb(strcmp(model.rxns,'EX_o2[e]')) = -1;
...
%        end
        ...
        elseif type_of_analysis == 7
            ...
            % ensure only one maximization objective NO WEIGHTED!
            if length(objective_function_reaction) ~= 1
                error('ERROR - General - Type of Analysis - Can only perform media sensitivity with maximizing one objective function reaction');
            elseif ~strcmp(objective_function_direction{1},'MAX')
                error('ERROR - General - Type of Analysis - Can only perform media sensitivity with maximizing one objective function reaction');
            ...
            % if 2 metabolites
                ...
                % find minimum exchanges value while maintaining objective value done first to ensure start_vals can be determined downstream 
                original_objective = find(model.c);
                original_objective_value = result.objval;
                original_objective_lb = model.lb(original_objective);
